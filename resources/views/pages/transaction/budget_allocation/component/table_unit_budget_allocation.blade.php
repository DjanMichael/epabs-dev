<table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_1">
    ...
    <tbody>
        ...
        // $used  = $row["actual_bal"];
        if (($hasBudget && $used != null) || ( $total != 0 && $used != null ) || $total != null){
        ...
        <tr>
            ...
            <td>
                <div class="d-flex flex-column w-100 mr-2">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <span class="text-muted font-size-sm font-weight-bold">₱ {{ number_format($row["yearly_pi_utilized"],2) }}</span>
                    </div>
            ...
            <td>
                <div class="d-flex flex-column w-100 mr-2">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <span class="text-muted mr-2 font-size-sm font-weight-bold">{{ $row["conap"] == 0 ? $bal : 0 }}%</span>
                        <span class="text-muted font-size-sm font-weight-bold">₱ {{ $row["conap"] == 0 ? number_format( $row["actual_bal"],2) : '0.00' }}</span>
                    </div>
                    <div class="progress progress-xs w-100">
                        <div class="progress-bar bg-danger" role="progressbar" style="width: {{ $row["conap"] == 0 ? $bal : 0 }}%;" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                </div>
            </td>
            <td>
                <div class="d-flex flex-column w-100 mr-2">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <span class="text-muted mr-2 font-size-sm font-weight-bold">{{ $row["conap"] != 0 ? number_format((($row["conap"]/$row["yearly_budget"]) * 100) ,2) : 0 }}%</span>
                        <span class="text-muted font-size-sm font-weight-bold">₱ {{ number_format(($row["conap"]),2) }}</span>
                    </div>
                    <div class="progress progress-xs w-100">
                        <div class="progress-bar bg-danger" role="progressbar" style="width: {{ $row["conap"] != 0 ? $bal : 0 }}%;" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>

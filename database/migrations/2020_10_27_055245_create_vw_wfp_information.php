        ...
        CREATE VIEW vw_wfp_activity_information AS (
            CREATE VIEW vw_wfp_activity_information AS (
                SELECT
                    ...
                    (
                        SELECT
                            (SELECT rfd.class_sequence FROM ref_function_deliverables rfd WHERE rfd.id = taof.output_function_id)
                        FROM
                    ...
                    (
                    SELECT
                            (SELECT rfd.function_class FROM ref_function_deliverables rfd WHERE rfd.id = taof.output_function_id)
                        FROM
                            tbl_activity_output_function taof
                        WHERE
                            `taof`.`id` = `twa`.`out_function`
                    ...
                    `twa`.`activity_category_id` AS `activity_category_id`,
                    (
                        SELECT
                            `rac`.`category`
                        FROM
                            `ref_activity_category` `rac`
                        WHERE
                            `rac`.`id` = `twa`.`activity_category_id`
                    ) AS `category`,
                    (
                        SELECT
                            `taof`.`description`
